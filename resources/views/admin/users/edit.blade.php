...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="email_confirm">{{ trans('cruds.user.fields.email_confirm') }}</label>
                <input class="form-control {{ $errors->has('email_confirm') ? 'is-invalid' : '' }}" type="email" name="email_confirm" id="email_confirm" value="{{ old('email_confirm', $user->email_confirm) }}">
                @if($errors->has('email_confirm'))
                    <span class="text-danger">{{ $errors->first('email_confirm') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.email_confirm_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('employee') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="employee" value="0">
                    <input class="form-check-input" type="checkbox" name="employee" id="employee" value="1" {{ $user->employee || old('employee', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="employee">{{ trans('cruds.user.fields.employee') }}</label>
                </div>
