...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.speakers.update", [$speaker->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="position">{{ trans('cruds.speaker.fields.position') }}</label>
                <textarea class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" name="position" id="position" required>{{ old('position', $speaker->position) }}</textarea>
                @if($errors->has('position'))
                    <span class="text-danger">{{ $errors->first('position') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.speaker.fields.position_helper') }}</span>
            </div>
            <div class="form-group">
